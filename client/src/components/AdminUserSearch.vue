  ...
  <b-container>
    ...
    <b-container id="searchRow">
      <b-row>
        <b-col>
          <b-form inline>
            <b-input v-model="searchQuery" placeholder="Search"
                     style="width:400px;margin-right:10px;"></b-input>
        ...
        <b-col sm="2">
          <b-button :disabled=isSearchButtonDisabled() class="float-right"
                    variant="light" v-on:click="searchUser()"
                    @click.prevent="updateUrl()">Search
          </b-button>
        </b-col>
          ...
          <b-col>
            <b-form-tags v-model="activityTypesForm.selectedOptions" no-outer-focus
              ...
              <template v-slot="{ tags, disabled, addTag, removeTag }">
                ...
                <b-dropdown block menu-class="w-100" size="sm" variant="outline-secondary">
        ...
        <b-row>
          <b-col>
            ...
            <b-form inline>
              ...
              <b-form-radio-group id="activityTypesSearchMethods" v-model="activityTypesForm.method"
    ...
    <b-row>
      <b-col>
        <template>
          <div>
            <b-modal id="deleteModal" ref="deleteModal" v-model="modalShow" hide-footer
                     @cancel="handleCancel" @ok="handleDelete">
              <div class="d-block text-center">
                <h3>Are you sure you want to delete this User?</h3>
              </div>
              <b-button class="modalBtn" size="lg" @click="handleCancel">
                Cancel
              </b-button>
              <b-button class="modalBtn" size="lg" variant="danger" @click="handleDelete">
                DELETE
              </b-button>
    ...
    <b-row v-if="this.data != null">
      ...
      <b-col>
        <b-form-select v-model="limit" :options="[10,20]" v-on:change="getUsers()"></b-form-select>
      </b-col>
    </b-row>
  </b-container>
</template>
...
export default {
  ...
  computed: {
    ...
    availableOptions() {
      ...
      const options = this.activityTypesForm.options.filter(
          opt => this.activityTypesForm.selectedOptions.indexOf(opt) === -1);
      if (criteria) {
  ...
  methods: {
    ...
    getUsers: function () {
      ...
      let query = '/profiles';
      if (this.searchBy !== 'email' && this.searchQuery.trim() !== ""
          && this.activityTypesForm.selectedOptions.length !== 0) {
    ...
    searchUser: function () {
      ...
      // this.currentPage = 1;
      if (this.searchQuery.trim() === '' && this.activityTypesForm.selectedOptions.length
          === 0) {
        return;
      }
